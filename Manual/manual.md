...
![HTML全体比較を設定した監視対象の追加画面](entry.webp)
...
HTML全体比較は、Webサーバーから取得したHTML文字列を加工せずに比較します。表示される文章だけでなく、HTMLコメント、タグ、属性、空白、改行の変化も更新として検知したい場合に使用します。

動作を試す場合は、次の値で監視対象を登録できます。
...
1. 「追加」をクリックします。
2. 上表の名前とURLを入力します。
3. 「監視モード」で「HTML全体比較」を選択します。
4. 「保存」をクリックします。
5. メイン画面で登録した行を選択し、「選択をチェック」をクリックします。

最初の正常なチェックは、比較の基準を保存するため「初回取得」になります。もう一度チェックし、取得したHTMLが同じなら「更新なし」、異なれば「更新あり」になります。

HTML全体比較は、ページの表示内容が同じでも、アクセスのたびに変わる属性値、生成時刻、コメント、空白などによって「更新あり」になることがあります。その場合はHTMLテキスト比較またはCSSセレクタ比較を使用してください。

### 5.3 CSSセレクタ比較を設定する

CSSセレクタ比較は、指定したCSSセレクタに一致する要素だけを比較します。日付、広告、閲覧数など、確認したい部分以外が頻繁に変わるページに向いています。

動作を試す場合は、次の値で監視対象を登録できます。`https://example.com/` の見出し要素に一致する `h1` を使用します。
...
![CSSセレクタ比較を設定した監視対象の追加画面](entry-css-selector.webp)

1. 「追加」をクリックします。
2. 上表の名前とURLを入力します。
3. 「監視モード」で「CSSセレクタ比較」を選択します。
4. 表示された「CSSセレクタ」欄へ `h1` を入力します。
5. 「保存」をクリックします。
6. メイン画面で登録した行を選択し、「選択をチェック」をクリックします。

最初の正常なチェックは「初回取得」になります。以後は、`h1` に一致した見出し要素が同じなら「更新なし」、要素の内容や属性が変われば「更新あり」になります。

CSSセレクタには、対象ページのHTMLに合わせて次のような値を指定できます。

| セレクタ例 | 対象 |
| --- | --- |
| `#news` | `id="news"` の要素 |
| `.news-list` | `class="news-list"` を持つすべての要素 |
| `main article` | `main` 要素内にあるすべての `article` 要素 |
| `ul.items > li` | `class="items"` の `ul` 要素の直下にあるすべての `li` 要素 |

一致した要素は、開始タグ、属性、内容、終了タグを含む外側HTMLとして比較されます。複数の要素が一致した場合は、ページ内の順番ですべてが比較対象になります。このため、一致要素の追加、削除、並び替え、属性変更も更新として検知されます。

CSSセレクタの書式が不正な場合や、一致する要素が一つもない場合は「エラー」になります。また、ArgusはJavaScriptを実行しないため、JavaScriptの実行後にだけ作成される要素は選択できません。エラーになっても、前回の正常な比較データは保持されます。
...
URL、監視モード、またはCSSセレクタを変更すると、それまで保存されていた比較用データは破棄されます。変更後に初めて正常にチェックしたときは「初回取得」になります。名前、有効状態、メモだけを変更した場合は、比較用データが引き継がれます。
...
ページ内の日付、アクセス数、広告、生成時刻など、閲覧のたびに変わる部分が比較対象に含まれている可能性があります。文章だけを確認したい場合はHTMLテキスト比較を使用してください。ページ内の一部分だけを確認したい場合はCSSセレクタ比較を選び、更新を確認したい要素の `id`、`class`、タグ名などをCSSセレクタへ指定してください。
