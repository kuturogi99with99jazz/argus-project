...
本書は、Avalonia を唯一の正式 UI とする Windows 専用デスクトップアプリ Argus の設計を、現行ソースコードを正本として定義します。2026-08-12 に PoC から正式採用へ移行し、2026-08-29 にmacOS対応を当面の対象外としました。WinForms 版は廃止します。

v0.2.0 には、手動チェック、3種類の比較方式、差分表示、設定のインポート・エクスポート、CSSセレクタ相談用プロンプトの生成・コピーが実装されています。自動監視、Windows OS標準ローカル通知、タスクバーバッジは未実装の将来拡張として定義します。
...
次のコンポーネントは現行ソースには含まれず、自動監視・通知を実装する際の将来設計です。
...
2. Coordinator が無効対象を除外し、同一対象の重複実行を許可する
...
- アプリ全体で共有する `HttpClient` を使用し、タイムアウトは30秒、リダイレクトは許可する
- HTTP要求の `User-Agent` は `Argus/0.1` とし、応答ヘッダーを受け取った時点で本文の読み込みを開始する
- 文字コードは、HTTPの `Content-Type` ヘッダー、BOM、HTML先頭8192バイト以内の `meta charset`、UTF-8フォールバックの順に判定する
- 指定された文字コードは厳格なデコーダーで処理し、未対応またはデコード不能な場合はチェックエラーとする
- `CheckCoordinator` のHTTP取得同時実行数は最大4件とし、超過分は待機する
...
- CSSセレクタ比較の入力欄に AI 相談ボタンを置き、URLと監視したい箇所の説明から生成したプロンプトを小窓へ表示する
...
- T-037では、設定のエクスポート、正常なインポート、形式エラー時の既存設定保持、インポート対象が初回取得になることを自動テストで検証し、Windowsのファイル選択と置換確認を手動確認する
...
- T-040では、URLと説明からのプロンプト生成、未入力時の案内、コピー成功・失敗、CSSモード以外での操作不可を自動テストし、Windowsで小窓・クリップボード・テーマ表示を手動確認する
