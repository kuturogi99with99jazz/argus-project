...
3. schema v1 JSONの読込、一覧、CRUD、全件／選択チェック、5状態（未確認を含む）、更新ありの差分表示、エラー時データ保持、ブラウザ起動を確認する
4. 設定のエクスポート、正常なインポート、置換確認、インポート後の初回取得状態、設定JSONのエラー時データ保持を確認する
5. CSSセレクタ比較時のAI相談小窓で、説明入力、プロンプト更新、コピー、キャンセル、および外部AIへ自動送信しないことを確認する
6. 「マニュアル」ボタンが未選択時と起動データエラー時にも利用でき、既定ブラウザで本文と2枚の画像を表示できることを確認する
7. ライト／ダーク、キーボード、ダイアログ、アイコン、`v0.2.0`、Releaseで`DEBUG`非表示を確認する
