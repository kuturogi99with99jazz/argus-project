...
「設定をエクスポート」で作成するポータブルJSONには、名前、URL、監視モード、CSSセレクタ、有効状態、メモだけが含まれます。UTF-8 BOMなしで保存し、日本語などの文字もJSON上で読める形のまま出力します。前回の比較データやチェック日時は含まれず、インポートした対象は次回の正常チェックで「初回取得」になります。
運用中のtargets.jsonと同じパスへはエクスポートできません。
...
今後は、設定した間隔での自動監視、更新あり時の Windows OS 標準ローカル通知、未確認件数のタスクバーアイコンバッジを追加する予定です。CSSセレクタ設定時のAI相談は、現行版でプロンプトの生成・表示・コピーに対応済みです。AIサービスへの自動送信や、LINE、Slack、メールなど外部サービスへの通知は行いません。
